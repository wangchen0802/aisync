...
import { CORS, json } from "@/lib/token";
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "用户说「同步到 SimReal / 同步给团队」时调用。由你直接整理这段对话里团队需要知道的内容：decision（已做的决定）、task（要做的事，带负责人和截止日期）、insight（有复用价值的事实或数据）、question（需要团队拍板的问题）。宁缺毋滥，通常 0–6 条，标题写结论本身。" +
      "如果对话推进了用户已有的任务，先调用 get_my_work 拿到任务编号，写进 task_updates，不要重复建任务。结果会进用户的收件箱（或按用户设置直接发布）。",
    inputSchema: {
      type: "object",
      properties: {
        title: { type: "string", description: "对话主题，≤20 字" },
        summary: { type: "string", description: "1–2 句话概括" },
        ...
        items: {
          ...
          items: {
            type: "object",
            properties: {
              kind: { type: "string", enum: ["decision", "task", "insight", "question"] },
              title: { type: "string", description: "一句话结论，≤40 字" },
              body: { type: "string", description: "一句话理由或补充，可空" },
              assignee: { type: "string", description: "任务负责人名字，可空" },
              due_date: { type: "string", description: "截止日期 YYYY-MM-DD，可空" },
        ...
        task_updates: {
          ...
          items: {
            ...
            properties: {
              ...
              note: { type: "string", description: "一句话进展" },
              done_subtasks: { type: "array", items: { type: "string" } },
            },
            required: ["task_id"],
          },
        },
        text: { type: "string", description: "对话原文或要点，可选，作者本人可见的存档" },
      },
    },
  },
];
...
async function callTool(me: Actor, name: string, a: Record<string, unknown>, clientName: string, origin: string): Promise<string> {
  ...
  switch (name) {
    ...
    case "sync_conversation": {
      const structured = Array.isArray(a.items) || Array.isArray(a.task_updates);
      const distilled = structured ? normalizeDistilled({ ...a, simreal_sync: "v1" }) : null;
      const text = String(a.text ?? "") || [a.title, a.summary].filter(Boolean).join("\n");
      if (!distilled && text.trim().length < 20) return "没有内容可同步：请把整理好的 items 传进来。";
      const r = await ingestConversation(me, { source: src, title: a.title ? String(a.title) : undefined, text, distilled });
  ...
  switch (msg.method) {
    case "initialize": {
      ...
      return ok({
        ...
        instructions: "SimReal 是团队的共享记忆。用户说「同步到 SimReal」时，由你整理对话里的决策、任务、洞察、问题，调用 sync_conversation 传入 items。开始任务前先调用 get_team_context（带上 topic）了解团队档案、本周目标和已确认的共识；拿不准时用 search_team_memory 查。做出决策后用 log_decision 记录，推进任务后用 update_task 更新进度。和投资人、客户、候选人沟通后用 log_outreach 记录，需要了解融资进度时用 get_pipeline。建议不要与「已确认的共识」冲突，冲突时要明确指出编号。",
      });
    }
    case "ping":
      return ok({});
...
export async function mcpPost(req: Request, me: Actor | null) {
